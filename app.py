...
if page == "Информация":
    ...
    if 'processed_data' in st.session_state:
        ...
        if 'materials_segments' in st.session_state and 'segments_stats' in st.session_state:
             ...
             st.markdown("""
             #### Распределение материалов по сегментам
             График ниже показывает, как материалы распределились по сегментам после анализа на основе заданных параметров (`Минимальное количество точек данных`, `Максимальный коэффициент вариации`, `Минимальное количество дней активности`).
             - **Подходит для прогнозирования:** Материалы с достаточным количеством исторических данных, относительно стабильными ценами и закупавшиеся на протяжении достаточного периода. Именно для этих материалов имеет смысл строить прогнозы цен.
             - **Недостаточно данных:** Материалы, по которым записей меньше, чем `Минимальное количество точек данных`. Прогнозирование по ним будет ненадежным из-за малого объема выборки.
             - **Нестабильный (Высокая волатильность):** Материалы, коэффициент вариации цен которых превышает `Максимальный коэффициент вариации`. Сильные колебания цен затрудняют прогнозирование.
             - **Недостаточно активный:** Материалы, период между первой и последней закупкой которых меньше, чем `Минимальное количество дней активности`. Короткая история не позволяет выявить тренды.

             *Бизнес-смысл:* Этот график дает общее представление о том, какая доля номенклатуры подходит для автоматического прогнозирования. Например, если большая часть материалов попадает в сегмент "Нестабильный", это может указывать на необходимость пересмотра закупочной политики или анализа причин такой волатильности для ключевых позиций.
             """)
             ...
             В таблице (или таблицах, если выбрано отображение по сегментам) ниже представлен список материалов, попавших в каждый сегмент. Для каждого материала указаны рассчитанные метрики: количество точек данных, коэффициент вариации (%) и длительность периода активности (дни).

             *Бизнес-смысл:* Эта таблица позволяет детально изучить состав каждого сегмента. Вы можете отсортировать или отфильтровать таблицу, чтобы найти конкретные материалы. Например, можно посмотреть, какие именно материалы попали в сегмент "Подходит для прогнозирования", чтобы сфокусироваться на них, или проанализировать материалы из сегмента "Недостаточно данных", чтобы оценить возможность сбора дополнительной информации по ним.
             """)
